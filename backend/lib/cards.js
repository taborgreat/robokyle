...
function cardPipeline({ match = {}, sort = 'new', skip = 0, limit = 20, viewerId = null }) {
  return [
    ...
    { $skip: skip },
    { $limit: limit },
    // Comments live in their own collection (models/Comment.js); count after
    // the page is cut so the lookup only runs for the cards being shown.
    { $lookup: {
    ...
    // "Produced N times" — arguably the strongest trust signal on the site
    // (Part II §4). Counts the sweep-maintained verified cache.
    { $lookup: {
