/* ============================================================
   ...
   so a muscle waypoint ["forearm", 12, -2.6, 1.6] is 12 cm down the
   ...
   (radius and ulna share it), "wrist" is the carpus, "index".."little"
   (or "mc2".."mc5") the finger metacarpals, "pp/mp/dp" proximal, middle
   and distal phalanges, "mc1/pp1/dp1" the thumb.
...
export const FINGERS = [
  ...
  { id: 2, key: 'index',  label: 'Index finger',  mc: 6.7, pp: 3.6, mp: 2.15, dp: 1.6,  base: [3.3,  1.35, 0.10], splay:  3 },
  { id: 3, key: 'middle', label: 'Middle finger', mc: 6.4, pp: 4.0, mp: 2.45, dp: 1.75, base: [3.4,  0.45, 0.00], splay:  0 },
  { id: 4, key: 'ring',   label: 'Ring finger',   mc: 5.7, pp: 3.75, mp: 2.35, dp: 1.65, base: [3.3, -0.55, 0.00], splay: -4 },
  { id: 5, key: 'little', label: 'Little finger', mc: 5.3, pp: 3.0, mp: 1.75, dp: 1.45, base: [3.1, -1.50, 0.10], splay: -9 },
];

export const THUMB = { mc: 4.5, pp: 2.9, dp: 2.0, base: [2.9, 1.95, 0.7], dir: [0.55, 0.75, 0.35] };
...
export const CARPALS = [
  { id: 'scaphoid',   name: 'Scaphoid',   pos: [1.2,  1.40,  0.25], r: [0.78, 0.58, 0.52],
  ...
  { id: 'lunate',     name: 'Lunate',     pos: [1.0,  0.15,  0.10], r: [0.58, 0.58, 0.58],
    text: 'Crescent-shaped, in the middle of the proximal row, sitting in the cup of the radius. Most wrist flexion and extension happens at the radius, lunate and scaphoid surface. The lunate is the carpal that dislocates most often.' },
  { id: 'triquetrum', name: 'Triquetrum', pos: [0.9, -1.15, -0.20], r: [0.58, 0.52, 0.48],
  ...
  { id: 'pisiform',   name: 'Pisiform',   pos: [0.7, -1.55,  0.62], r: [0.42, 0.42, 0.36],
  ...
  { id: 'trapezium',  name: 'Trapezium',  pos: [2.6,  1.95,  0.55], r: [0.62, 0.58, 0.52],
  ...
  { id: 'trapezoid',  name: 'Trapezoid',  pos: [2.7,  0.95,  0.20], r: [0.48, 0.48, 0.48],
  ...
  { id: 'capitate',   name: 'Capitate',   pos: [2.7,  0.00,  0.00], r: [0.68, 0.68, 0.62],
  ...
  { id: 'hamate',     name: 'Hamate',     pos: [2.6, -1.15,  0.00], r: [0.62, 0.62, 0.58],
...
export const JOINT_TEXT = {
  shoulder: { name: 'Shoulder (glenohumeral) joint', type: 'Ball and socket',
    ...
    text: 'The humeral head is much bigger than the shallow socket it sits in, so mobility is bought with instability. The rotator cuff muscles hold the ball centred while the deltoid lifts the arm; neither is drawn in this model. Drag the handle to aim the upper arm, Shift-drag to roll it.' },
  elbow: { name: 'Elbow (humeroulnar) joint', type: 'Hinge',
    motion: 'Flexion 0 to 145 degrees.',
  ...
  pronation: { name: 'Radioulnar joints (pronation / supination)', type: 'Pivot (two joints working together)',
    motion: 'Pronation about 75 degrees, supination about 85 degrees.',
    text: 'The radius spins in a ligament ring at the elbow and swings around the ulna at the wrist, carrying the hand with it: palm down is pronation, palm up is supination. Pronator teres and pronator quadratus turn the palm down; biceps and supinator turn it up. In the model the whole forearm rotates as one piece; the real crossing of the two bones is simplified. Drag the handle sideways to turn it.' },
  wrist: { name: 'Wrist (radiocarpal) joint', type: 'Ellipsoid (condyloid)',
    motion: 'Flexion about 75 degrees, extension about 70, radial deviation about 20, ulnar deviation about 35.',
    text: 'The scaphoid and lunate sit in the cup of the radius; the ulna is separated from the carpus by a cartilage disc. Flexion and extension are shared between this joint and the midcarpal joint between the two carpal rows. Drag the handle up and down to flex and extend, sideways to tilt.' },
  cmc1: { name: 'Thumb carpometacarpal joint', type: 'Saddle',
    ...
    text: 'Two saddle-shaped surfaces at right angles: the base of the first metacarpal rocking on the trapezium. It is the joint that lets the thumb pad meet each fingertip, and the joint that wears out first in many hands. Drag the handle up and down to swing the thumb away from the palm, sideways to sweep it across.' },
  mcp1: { name: 'Thumb metacarpophalangeal joint', type: 'Hinge (with a little side play)', motion: 'Flexion about 55 degrees.',
  ...
  ip1: { name: 'Thumb interphalangeal joint', type: 'Hinge', motion: 'Flexion about 80 degrees, some hyperextension.',
  ...
  mcp: { name: 'Metacarpophalangeal joint', type: 'Condyloid', motion: 'Flexion about 90 degrees, extension about 30, spread 20 either way.',
  ...
  pip: { name: 'Proximal interphalangeal joint', type: 'Hinge', motion: 'Flexion about 100 degrees.',
  ...
  dip: { name: 'Distal interphalangeal joint', type: 'Hinge', motion: 'Flexion about 70 degrees.',
...
/* ------------------------------------------------------------ muscles
   ...
   Thick where the belly is, thin where it becomes tendon. `nerves` names
...
export const MUSCLES = [
  /* ----- upper arm ----- */
  { id: 'biceps', name: 'Biceps brachii', group: 'arm', nerves: ['mcn'],
  ...
  { id: 'brachialis', name: 'Brachialis', group: 'arm', nerves: ['mcn', 'radial'],
  ...
  { id: 'triceps', name: 'Triceps brachii', group: 'arm', nerves: ['radial'],
    ...
    action: 'Extends the elbow; the long head also helps extend the shoulder.', nerve: 'Radial nerve (C6 to C8).',
  ...
  { id: 'pt', name: 'Pronator teres', group: 'flexors', nerves: ['median'],
  ...
  { id: 'fcr', name: 'Flexor carpi radialis', group: 'flexors', nerves: ['median'],
  ...
  { id: 'pl', name: 'Palmaris longus', group: 'flexors', nerves: ['median'],
  ...
  { id: 'fcu', name: 'Flexor carpi ulnaris', group: 'flexors', nerves: ['ulnar'],
  ...
  { id: 'fds', name: 'Flexor digitorum superficialis', group: 'flexors', nerves: ['median'],
    strands: [
  ...
  { id: 'fdp', name: 'Flexor digitorum profundus', group: 'flexors', nerves: ['median', 'ulnar'],
    strands: [
  ...
  { id: 'fpl', name: 'Flexor pollicis longus', group: 'flexors', nerves: ['median'],
  ...
  { id: 'pq', name: 'Pronator quadratus', group: 'flexors', nerves: ['median'],
  ...
  { id: 'br', name: 'Brachioradialis', group: 'extensors', nerves: ['radial'],
  ...
  { id: 'ecrl', name: 'Extensor carpi radialis longus', group: 'extensors', nerves: ['radial'],
  ...
  { id: 'ecrb', name: 'Extensor carpi radialis brevis', group: 'extensors', nerves: ['radial'],
  ...
  { id: 'ed', name: 'Extensor digitorum', group: 'extensors', nerves: ['radial'],
    strands: [
  ...
  { id: 'edm', name: 'Extensor digiti minimi', group: 'extensors', nerves: ['radial'],
    strands: [{ pts: [['humerus', 29.7, 1.6, -0.9], ['forearm', 6, -0.8, -2.3], ['forearm', 14, -1.2, -2.0], ['forearm', 22, -1.4, -1.3], ['wrist', 2, -1.3, -0.85], ['little', 0.5, -0.1, -0.66], ['little', 'len-0.3', 0, -0.62], ['pp5', 'len-0.4', 0, -0.47]],
  ...
  { id: 'ecu', name: 'Extensor carpi ulnaris', group: 'extensors', nerves: ['radial'],
    strands: [{ pts: [['humerus', 29.8, 1.3, -1.1], ['forearm', 6, -1.8, -2.0], ['forearm', 14, -2.2, -1.7], ['forearm', 22, -2.2, -1.1], ['wrist', 1.2, -1.7, -0.6], ['little', 0.4, -0.1, -0.5]],
  ...
  { id: 'supinator', name: 'Supinator', group: 'extensors', nerves: ['radial'],
  ...
  { id: 'apl', name: 'Abductor pollicis longus', group: 'extensors', nerves: ['radial'],
    strands: [{ pts: [['forearm', 10, -0.5, -1.6], ['forearm', 16, 1.2, -1.4], ['forearm', 22, 2.4, -0.6], ['wrist', 1.5, 2.5, 0.2], ['mc1', 0.4, 0.15, 0]],
  ...
  { id: 'epb', name: 'Extensor pollicis brevis', group: 'extensors', nerves: ['radial'],
    strands: [{ pts: [['forearm', 13, 0.6, -1.7], ['forearm', 19, 1.6, -1.4], ['forearm', 24, 2.4, -0.6], ['wrist', 2.2, 2.5, 0], ['mc1', 'len*0.5', 0.2, -0.36], ['pp1', 0.4, 0, -0.4]],
  ...
  { id: 'epl', name: 'Extensor pollicis longus', group: 'extensors', nerves: ['radial'],
    strands: [{ pts: [['forearm', 11, -1.0, -1.8], ['forearm', 18, -0.2, -1.9], ['forearm', 24, 1.0, -1.25], ['wrist', 2.4, 2.0, -0.5], ['mc1', 'len*0.5', 0, -0.46], ['pp1', 'len*0.5', 0, -0.4], ['dp1', 0.45, 0, -0.28]],
  ...
  { id: 'ei', name: 'Extensor indicis', group: 'extensors', nerves: ['radial'],
    strands: [{ pts: [['forearm', 14, -0.6, -1.7], ['forearm', 22, 0.4, -1.4], ['wrist', 2.5, 1.0, -0.85], ['index', 0.5, 0, -0.7], ['index', 'len-0.3', 0, -0.64], ['pp2', 0.6, 0, -0.5]],
  ...
  { id: 'apb', name: 'Abductor pollicis brevis', group: 'hand', nerves: ['median'],
    strands: [{ pts: [['wrist', 2.2, 1.8, 1.1], ['mc1', 'len*0.5', 0.9, 0.55], ['pp1', 0.3, 0.5, 0.2]], r: [[0, 0.3], [0.5, 0.58], [1, 0.2]] }],
  ...
  { id: 'fpb', name: 'Flexor pollicis brevis', group: 'hand', nerves: ['median', 'ulnar'],
    strands: [{ pts: [['wrist', 2.6, 1.5, 1.2], ['mc1', 'len*0.6', 0.2, 0.9], ['pp1', 0.3, -0.3, 0.3]], r: [[0, 0.3], [0.5, 0.58], [1, 0.2]] }],
  ...
  { id: 'opp', name: 'Opponens pollicis', group: 'hand', nerves: ['median'],
    strands: [{ pts: [['wrist', 2.5, 1.7, 0.8], ['mc1', 'len*0.4', 0.75, 0.3], ['mc1', 'len*0.85', 0.65, 0.2]], r: [[0, 0.3], [0.5, 0.52], [1, 0.25]] }],
  ...
  { id: 'adp', name: 'Adductor pollicis', group: 'hand', nerves: ['ulnar'],
    strands: [{ pts: [['middle', 2.0, 0, 0.6], ['index', 3.6, 0.4, 1.0], ['mc1', 'len*0.9', -0.6, 0.5], ['pp1', 0.3, -0.5, 0.3]], r: [[0, 0.35], [0.4, 0.7], [1, 0.22]] }],
  ...
  { id: 'adm', name: 'Abductor digiti minimi', group: 'hand', nerves: ['ulnar'],
    strands: [{ pts: [['wrist', 0.9, -1.85, 0.6], ['little', 'len*0.5', -0.8, 0.3], ['pp5', 0.3, -0.42, 0.1]], r: [[0, 0.3], [0.5, 0.52], [1, 0.2]] }],
  ...
  { id: 'fdm', name: 'Flexor digiti minimi brevis', group: 'hand', nerves: ['ulnar'],
    strands: [{ pts: [['wrist', 2.6, -1.4, 0.7], ['little', 'len*0.5', -0.2, 0.7], ['pp5', 0.3, -0.2, 0.3]], r: [[0, 0.25], [0.5, 0.46], [1, 0.18]] }],
  ...
  { id: 'odm', name: 'Opponens digiti minimi', group: 'hand', nerves: ['ulnar'],
    strands: [{ pts: [['wrist', 2.7, -1.5, 0.4], ['little', 'len*0.5', -0.65, 0.0], ['little', 'len*0.9', -0.55, 0.0]], r: [[0, 0.3], [0.5, 0.46], [1, 0.22]] }],
  ...
  ...[2, 3, 4, 5].map(i => ({
    id: `lumb${i - 1}`, name: `Lumbrical ${['I', 'II', 'III', 'IV'][i - 2]} (${fingerFrames[i]} finger)`, group: 'hand', nerves: [i <= 3 ? 'median' : 'ulnar'],
    strands: [{ pts: [[fingerFrames[i], 2.0, 0.0, 0.8], [fingerFrames[i], 'len-0.5', 0.42, 0.52], [`pp${i}`, 1.1, 0.4, -0.05]], r: [[0, 0.22], [0.5, 0.33], [1, 0.14]] }],
  ...
  ...[
    { n: 1, frame: 'index',  y: 0.55,  pp: 'pp2', side: 0.38 },
    { n: 2, frame: 'middle', y: 0.5,   pp: 'pp3', side: 0.38 },
    { n: 3, frame: 'middle', y: -0.5,  pp: 'pp3', side: -0.38 },
    { n: 4, frame: 'ring',   y: -0.5,  pp: 'pp4', side: -0.38 },
  ].map(d => ({
    id: `di${d.n}`, name: `Dorsal interosseous ${['I', 'II', 'III', 'IV'][d.n - 1]}`, group: 'hand', nerves: ['ulnar'],
    strands: [{ pts: [[d.frame, 1.2, d.y, -0.1], [d.frame, 'len-0.6', d.y * 0.85, -0.28], [d.pp, 0.6, d.side, -0.3]], r: [[0, 0.3], [0.45, 0.5], [1, 0.16]] }],
  ...
  ...[
    { n: 1, frame: 'index',  y: -0.42, pp: 'pp2', side: -0.32 },
    { n: 2, frame: 'ring',   y: 0.42,  pp: 'pp4', side: 0.32 },
    { n: 3, frame: 'little', y: 0.42,  pp: 'pp5', side: 0.32 },
  ].map(d => ({
    id: `pi${d.n}`, name: `Palmar interosseous ${['I', 'II', 'III'][d.n - 1]}`, group: 'hand', nerves: ['ulnar'],
    strands: [{ pts: [[d.frame, 1.5, d.y, 0.38], [d.frame, 'len-0.6', d.y * 0.9, 0.32], [d.pp, 0.6, d.side, 0.1]], r: [[0, 0.25], [0.45, 0.38], [1, 0.14]] }],
...
/* Group colours are deliberately different hues, not just shades, so the
   four compartments can be told apart at a glance. */
export const MUSCLE_GROUPS = {
  arm:       { label: 'Upper arm',         color: 0x9a2c2c },
  flexors:   { label: 'Forearm flexors',   color: 0xb0323f },
  extensors: { label: 'Forearm extensors', color: 0x8a3563 },
  hand:      { label: 'Hand intrinsics',   color: 0xc9573b },
};

/* ------------------------------------------------------------ nerves */
export const NERVES = [
  { id: 'median', name: 'Median nerve', roots: 'C5 to T1, from the lateral and medial cords of the brachial plexus',
    strands: [
      ...
      { label: 'recurrent (thenar) branch', pts: [['wrist', 3.2, 0.3, 1.0], ['wrist', 3.1, 1.5, 1.3], ['mc1', 1.0, 0.5, 0.7]], r: [[0, 0.12], [1, 0.08]] },
      { label: 'anterior interosseous branch', pts: [['forearm', 8, 0.2, 1.45], ['forearm', 14, -0.2, 0.62], ['forearm', 21, -0.2, 0.5]], r: [[0, 0.14], [1, 0.08]] },
      { label: 'digital branch, thumb', pts: [['wrist', 3.2, 0.3, 1.0], ['mc1', 'len*0.5', -0.3, 0.8], ['pp1', 'len-0.3', 0, 0.55], ['dp1', 0.6, 0, 0.35]], r: [[0, 0.13], [1, 0.07]] },
      { label: 'digital branch, index', pts: [['wrist', 3.2, 0.3, 1.0], ['index', 2, 0.2, 0.9], ['pp2', 2, 0.2, 0.58], ['dp2', 0.6, 0.1, 0.32]], r: [[0, 0.13], [1, 0.07]] },
      { label: 'digital branch, middle', pts: [['wrist', 3.2, 0.3, 1.0], ['middle', 2, 0, 0.9], ['pp3', 2, 0, 0.58], ['dp3', 0.6, 0, 0.32]], r: [[0, 0.13], [1, 0.07]] },
      { label: 'digital branch, ring (radial half)', pts: [['wrist', 3.2, 0.3, 1.0], ['ring', 2, 0.3, 0.9], ['pp4', 2, 0.3, 0.58], ['dp4', 0.6, 0.2, 0.32]], r: [[0, 0.12], [1, 0.07]] },
    ],
  ...
  { id: 'ulnar', name: 'Ulnar nerve', roots: 'C8 to T1, from the medial cord of the brachial plexus',
    strands: [
      { pts: [['humerus', 1, -1.8, 0.8], ['humerus', 12, -2.2, 0.3], ['humerus', 24, -2.6, -0.5], ['humerus', 29.6, -2.9, -0.6], ['forearm', 3, -2.6, 0.6], ['forearm', 10, -2.6, 1.2], ['forearm', 18, -2.3, 1.3], ['forearm', 24, -1.9, 1.2], ['wrist', 1.2, -1.5, 1.2], ['wrist', 2.8, -1.1, 1.1]],
        r: [[0, 0.28], [0.8, 0.24], [1, 0.2]] },
      { label: 'deep (motor) branch', pts: [['wrist', 2.8, -1.1, 1.1], ['ring', 1.5, -0.2, 0.7], ['middle', 2.5, 0, 0.6], ['index', 3.0, 0.4, 0.8]], r: [[0, 0.14], [1, 0.08]] },
      { label: 'digital branch, little', pts: [['wrist', 2.8, -1.1, 1.1], ['little', 2, -0.2, 0.8], ['pp5', 1.5, 0, 0.52], ['dp5', 0.5, 0, 0.32]], r: [[0, 0.13], [1, 0.07]] },
      { label: 'digital branch, ring (ulnar half)', pts: [['wrist', 2.8, -1.1, 1.1], ['ring', 2, -0.3, 0.9], ['pp4', 2, -0.3, 0.58], ['dp4', 0.6, -0.2, 0.32]], r: [[0, 0.12], [1, 0.07]] },
      { label: 'dorsal cutaneous branch', pts: [['forearm', 20, -2.3, 0.9], ['forearm', 24, -2.5, -0.8], ['wrist', 2, -1.7, -0.85], ['little', 1.5, -0.3, -0.75]], r: [[0, 0.12], [1, 0.07]] },
    ],
  ...
  { id: 'radial', name: 'Radial nerve', roots: 'C5 to T1, from the posterior cord of the brachial plexus',
    strands: [
      { pts: [['humerus', 1, -1.0, -0.6], ['humerus', 10, -0.6, -2.7], ['humerus', 18, 1.4, -2.5], ['humerus', 25, 2.6, -0.6], ['humerus', 29.5, 2.6, 0.8], ['forearm', 2.5, 2.6, 1.2]],
        r: [[0, 0.3], [1, 0.22]] },
      { label: 'superficial (sensory) branch', pts: [['forearm', 2.5, 2.6, 1.2], ['forearm', 10, 3.0, 0.6], ['forearm', 18, 2.8, -0.2], ['forearm', 24, 2.4, -0.95], ['wrist', 2, 1.9, -0.95], ['index', 1.5, 0.5, -0.8], ['pp2', 1.5, 0.3, -0.52]], r: [[0, 0.16], [1, 0.08]] },
      { label: 'superficial branch, thumb', pts: [['wrist', 2, 1.9, -0.95], ['mc1', 'len*0.5', 0.3, -0.55], ['pp1', 0.8, 0.2, -0.42]], r: [[0, 0.1], [1, 0.07]] },
  ...
  { id: 'mcn', name: 'Musculocutaneous nerve', roots: 'C5 to C7, from the lateral cord of the brachial plexus',
...
/* ------------------------------------------------------------ presets
   Named hand positions. Values are degrees, or a 0 to 1 "curl" for a whole
